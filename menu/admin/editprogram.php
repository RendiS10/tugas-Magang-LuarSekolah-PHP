...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM program_pelatihan WHERE id_program = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $query = "UPDATE program_pelatihan SET nama_program = ?, deskripsi = ?, durasi = ?, level = ? WHERE id_program = ?";
        $stmt = $koneksi->prepare($query);
        $stmt->bind_param("ssssi", $namaProgram, $deskripsi, $durasi, $level, $idProgram);
...
<head>
    ...
    <title>Edit Program Pelatihan</title>
